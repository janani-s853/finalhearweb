...
const HearingTest = () => {
    ...
    const calculateSummary = async (data) => {
        ...
        await saveHearingTestResults(data, testResult);
    };

    const saveHearingTestResults = async (testData, overallResult) => {
        try {
            setSaveError("");
            
            if (!user) {
                setSaveError("Please log in to save your hearing test results.");
                console.error('User not authenticated');
                return;
            }
            
            console.log('Saving hearing test for user:', user.id);
                ...
                .insert([
                    {
                        ...
                        left_ear_results: JSON.stringify(testData),
                        right_ear_results: JSON.stringify(testData),
                        overall_score: testData.filter(d => d.heard).length,
                        test_type: 'online_audiometry',
                    }
                ])
                .select();
            ...
            if (error) {
                ...
                setSaveError(`Failed to save test results: ${error.message}`);
            } else {
                console.log('Hearing test results saved successfully:', data);
            }
        } catch (error) {
            console.error('Error saving hearing test results:', error);
            setSaveError(`Failed to save test results: ${error.message}`);
